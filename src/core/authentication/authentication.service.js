...
import {
	...
	Forbidden,
	NotFound,
} from '../../exceptions/catch.execption.js';
...
class AuthenticationService extends BaseService {
	...
	login = async (payload) => {
		const user = await this.db.user.findUnique({
			where: { email: payload.email, isSuspended: false },
			include: {
		...
		return {
			...
			...(!user.member.dataVerified && user.role.code != 'ADMIN'
				? { access: false }
				: { access: true }),
	...
	forgotPassword = async (payload) => {
		...
		forgetExpiry.setHours(
			forgetExpiry.getHours() + 1 - forgetExpiry.getTimezoneOffset() / 60
		);

		await this.db.user.update({
			where: { id: user.id },
			data: { forgetToken, forgetExpiry },
		});
	...
	resetPassword = async (payload) => {
		...
		const user = await this.db.user.findFirst({
			where: { forgetToken, id: decoded.uid },
		});
		if (!user || user.forgetExpiry < new Date())
			throw new BadRequest('Token kadaluwaras atau tidak valid');
